...
from bear.utils import compat_bytes
...
class Operation:
    ...
    def get_class(class_name):
        ...
        module = importlib.import_module('bearbase.operations')
        ...
        if isArgsThisClass(self, args):
            ...
            super(WitnessProps, self).__init__(
                OrderedDict([
                    ...
                    ('bsd_interest_rate', Uint16(kwargs["bsd_interest_rate"])),
                ]))
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            super(WithdrawVesting, self).__init__(
                OrderedDict([
                    ('account', String(kwargs["account"])),
                    ('coining_shares', Amount(kwargs["coining_shares"])),
                ]))
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            super(ClaimRewardBalance, self).__init__(
                OrderedDict([
                    ('account', String(kwargs["account"])),
                    ('reward_bear', Amount(kwargs["reward_bear"])),
                    ('reward_sbd', Amount(kwargs["reward_sbd"])),
                    ('reward_vests', Amount(kwargs["reward_vests"])),
                ]))


class DelegateCoiningShares(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            if len(args) == 1 and len(kwargs) == 0:
                kwargs = args[0]
            super(DelegateCoiningShares, self).__init__(
                OrderedDict([
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if not kwargs["block_signing_key"]:
                kwargs[
                    "block_signing_key"] = \
                    "SHR1111111111111111111111111111111114T1Anm"
            ...
            super(CommentOptions, self).__init__(
                OrderedDict([
                    ...
                    ('percent_bear_dollars',
                     Uint16(int(kwargs["percent_bear_dollars"]))),
