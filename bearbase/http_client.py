...
import certifi
import urllib3
from bearbase.exceptions import RPCError, RPCErrorRecoverable
...
class HttpClient(object):
    """ Simple Bear JSON-HTTP-RPC API

    This class serves as an abstraction layer for easy use of the Bear API.

    Args:
      nodes (list): A list of Bear HTTP RPC nodes to connect to.

    .. code-block:: python

       from bear.http_client import HttpClient

       rpc = HttpClient(['https://beard-node1.com',
       'https://beard-node2.com'])
    ...
    def json_rpc_body(name, *args, **kwargs):
        """ Build request body for beard RPC requests.
    ...
    def call(self,
             ...
             *args,
             **kwargs):
        """ Call a remote procedure in beard.
        ...
        while True:
            ...
            #       define exceptions for which we refuse to retry.
                ...
                logger.error('Unexpected exception! Please report at ' +
                             'https://github.com/bearshares/bear-python/issues' +
    ...
    def sanitize_nodes(self, nodes):
        ...
        1. a string of a single node url. ie nodes='https://api.bearshares.com'
        2. a comma separated string of several node url's.
            nodes='https://api.bearshares.com,<<community node>>,<<your personal node>>'
