...
from bearbase.exceptions import AccountDoesNotExistsException
...
class Account(dict):
    ...
    def __init__(self, account_name, beard_instance=None):
    ...
    def refresh(self):
        account = self.beard.get_account(self.name)
        if not account:
    ...
    def converter(self):
        if not self._converter:
            self._converter = Converter(self.beard)
    ...
    def get_balances(self):
        ...
        totals = {
            ...
            'BSD':
                sum([available['BSD'], savings['BSD'], rewards['BSD']]),
            'COINS':
                sum([available['COINS'], rewards['COINS']]),
        }
    ...
    def _get_followers(self, direction="follower", last_user=""):
        if direction == "follower":
            ...
            followers = self.beard.get_followers(self.name, last_user, "blog",
                                                  100)
        elif direction == "following":
            followers = self.beard.get_following(self.name, last_user, "blog",
                                                  100)
    ...
    def virtual_op_count(self):
        try:
            last_item = self.beard.get_account_history(self.name, -1, 0)[0][0]
        except IndexError:
            return 0
        else:
            return last_item
    ...
    def get_account_votes(self):
        return self.beard.get_account_votes(self.name)

    def get_withdraw_routes(self):
        return self.beard.get_withdraw_routes(self.name, 'all')

    def get_conversion_requests(self):
        return self.beard.get_conversion_requests(self.name)
    ...
    def get_account_history(self,
                            ...
                            filter_by=None,
                            raw_output=False):
        """ A generator over beard.get_account_history.
        ...
        Args:
            ...
            raw_output (bool): (Defaults to False). If True, return history in
                beard format (unchanged).
        """
        history = self.beard.get_account_history(self.name, index, limit)
