...
from bearbase.chains import known_chains
from bearbase.http_client import HttpClient
from bearbase.storage import configStorage
from bearbase.transactions import SignedTransaction
from bearbase.types import PointInTime
...
class Beard(HttpClient):
    ...
    def __init__(self, nodes=None, **kwargs):
        if not nodes:
            nodes = get_config_node_list() or ['https://api.bearshares.com']

        super(Beard, self).__init__(nodes, **kwargs)
    ...
    def get_replies(self, author, skip_own=True):
        ...
        for reply in replies:
            ...
            if skip_own and post["author"] == author:
                continue
            discussions.append(Post(post, beard_instance=self))
        return discussions
    ...
    def get_promoted(self):
        ...
        for p in promoted:
            ...
            r.append(Post(post, beard_instance=self))
        return r
    ...
    def get_posts(self, limit=10, sort="hot", category=None, start=None):
        ...
        for p in func(discussion_query):
            r.append(Post(p, beard_instance=self))
        return r
    ...
    def stream_comments(self, *args, **kwargs):
        ...
        for c in Blockchain(
            ...
            yield Post(c, beard_instance=self)
    ...
    def get_account(self, account):
        ...
        Args:
            account (str): BEARS username that we are looking up.
    ...
    def get_all_usernames(self, last_user=''):
        """ Fetch the full list of BEARS usernames. """
    ...
    def _get_blocks(self, blocks):
        """ Fetch multiple blocks from beard at once.
    ...
    def get_blocks(self, block_nums):
        """ Fetch multiple blocks from beard at once, given a range.
        ...
        Example:
            ...
            ::
                ...
                {'content_constant': '2000000000000',
                 ...
                 'reward_balance': '555660.895 BEARS'}

        """
        return self.call('get_reward_funds', fund_name, api='database_api')

    def get_expiring_coining_delegations(self, account,
                                         start, limit):
    ...
    def get_chain_properties(self):
        ...
        ::
            ...
            {'account_creation_fee': '30.000 BEARS',
             'maximum_block_size': 65536,
             'bsd_interest_rate': 250}

        """
        return self.call('get_chain_properties', api='database_api')

    def get_feed_history(self):
        """ Get the hourly averages of witness reported BEARS/BSD prices.

        ::

            {'current_median_history':
                {'base': '0.093 BSD', 'quote': '1.010 BEARS'},
             'id': 0,
             'price_history': [{'base': '0.092 BSD', 'quote': '1.010 BEARS'},
              {'base': '0.093 BSD', 'quote': '1.020 BEARS'},
              {'base': '0.093 BSD', 'quote': '1.010 BEARS'},
              {'base': '0.094 BSD', 'quote': '1.020 BEARS'},
              {'base': '0.093 BSD', 'quote': '1.010 BEARS'},
    ...
    def get_current_median_history_price(self):
        """ Get the average BEARS/BSD price.
    ...
    def get_account_count(self):
        """ How many accounts are currently registered on BEARS? """
                ...
                {'block': 9942005,
                 'op': ['curation_reward',
                  {'comment_author': 'leongkhan',
                   'comment_permlink': 'bear-investor-report-5-march-2017',
                   'curator': 'furion',
    ...
    def get_active_votes(self, author, permlink):
        ...
        Args:
            author (str): OP's BEARS username.
        ...
        Example:
            .. code-block:: python
               ...
               'bearifying-idioms-there-s-no-use-crying-over-spilt-milk')
    ...
    def get_coining_delegations(self, account, from_account,
                                limit):
    ...
    def get_version(self):
        """ Get beard version of the node currently connected to. """
    ...
    def get_ticker(self):
        """ Returns the market ticker for the internal BSD:BEARS market. """
    ...
    def get_trade_history(self, start, end,
                          limit):
        """ Returns the trade history for the internal BSD:BEARS market. """
        return self.call(
            'get_trade_history', start, end, limit, api='market_history_api')

    def get_recent_trades(self, limit):
        """ Returns the N most recent trades for the internal BSD:BEARS
        market. """
    ...
    def get_market_history(self, bucket_seconds, start,
        ...
        """ Returns the market history for the internal BSD:BEARS market. """
        return self.call(
            'get_market_history',
            bucket_seconds,
            start,
