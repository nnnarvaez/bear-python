...
from bearbase import memo
from bearbase import operations
from bearbase.account import PrivateKey, PublicKey
from bearbase.exceptions import AccountExistsException, MissingKeyError
...
BEARSIT_100_PERCENT = 10000
BEARSIT_1_PERCENT = (BEARSIT_100_PERCENT / 100)
...
class Commit(object):
    """ Commit things to the Bear network.
        ...
        :param Beard beard: Beard node to connect to*
        ...
        * **Wallet Database**: Here, the bearlibs load the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call ``Bear()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``Bear()``.
    ...
    def __init__(self, beard_instance=None, no_broadcast=False, **kwargs):
        ...
        self.wallet = Wallet(self.beard, **kwargs)
    ...
    def broadcast(self, signed_trx):
        """ Broadcast a transaction to the Bear network
    ...
    def post(self,
        ...
        if comment_options or beneficiaries:
            options = keep_in_dict(comment_options or {}, [
                'max_accepted_payout', 'percent_bear_dollars', 'allow_votes',
            ...
            default_max_payout = "1000000.000 BSD"
            comment_op = operations.CommentOptions(
                **{
                    ...
                    "percent_bear_dollars":
                        int(options.get("percent_bear_dollars", 10000)),
                    "allow_votes":
    ...
    def vote(self, identifier, weight, account=None):
        """ Vote for a post
            ...
            .. code-block:: python
                ...
                bearpy set default_account <account>
        """
        if not account:
        ...
        op = operations.Vote(
            **{
                ...
                "weight": int(weight * BEARSIT_1_PERCENT)
            })
    ...
    def create_account(
            ...
            delegation_fee_bear='0 BEARS',
            creator=None,
    ):
        """ Create new account in Bear

            The brainkey/password can be used to recover all generated keys
            (see `bearbase.account` for more details.
            ...
            .. note:: Account creations cost a fee that is defined by
               ...
               **You can partially pay that fee by delegating VESTS.**
               ...
               To pay the fee in full in BEARS, leave
               ``delegation_fee_bear`` set to ``0 BEARS`` (Default).

               To pay the fee partially in BEARS, partially with delegated
               VESTS, set ``delegation_fee_bear`` to a value greater than ``1
               BEARS``. `Required VESTS will be calculated automatically.`

               To pay the fee with maximum amount of delegation, set
               ``delegation_fee_bear`` to ``1 BEARS``. `Required VESTS will be
        ...
        you lose your password!
            ...
            :param str delegation_fee_bear: (Optional) If set, `creator` pay a
            fee of this amount, and delegate the rest with VESTS (calculated
            automatically). Minimum: 1 BEARS. If left to 0 (Default), full fee
        ...
        if not creator:
            raise ValueError(
                ...
                "creator=x, or set the default_account using bearpy")
        ...
        try:
            Account(account_name, beard_instance=self.beard)
        ...
        from bearbase.account import PasswordKey, PublicKey
        if password:
            ...
            posting_pubkey = PublicKey(
                posting_key, prefix=self.beard.chain_params["prefix"])
            active_pubkey = PublicKey(
                active_key, prefix=self.beard.chain_params["prefix"])
            owner_pubkey = PublicKey(
                owner_key, prefix=self.beard.chain_params["prefix"])
            memo_pubkey = PublicKey(
                memo_key, prefix=self.beard.chain_params["prefix"])
        else:
            raise ValueError(
                "Call incomplete! Provide either a password or public keys!")

        owner = format(owner_pubkey, self.beard.chain_params["prefix"])
        active = format(active_pubkey, self.beard.chain_params["prefix"])
        posting = format(posting_pubkey, self.beard.chain_params["prefix"])
        memo = format(memo_pubkey, self.beard.chain_params["prefix"])
        ...
        props = self.beard.get_chain_properties()
        required_fee_bear = Amount(props["account_creation_fee"]).amount * 30

        required_fee_vests = 0
        delegation_fee_bear = Amount(delegation_fee_bear).amount
        if delegation_fee_bear:
            # creating accounts without delegation requires 30x
            # account_creation_fee creating account with delegation allows one
            # to use VESTS to pay the fee where the ratio must satisfy 1 BEARS
            # in fee == 5 BEARS in delegated VESTS

            delegated_sp_fee_mult = 5

            if delegation_fee_bear < 1:
                raise ValueError(
                    "When creating account with delegation, at least " +
                    "1 BEARS in fee must be paid.")

            # calculate required remaining fee in vests
            remaining_fee = required_fee_bear - delegation_fee_bear
            if remaining_fee > 0:
        ...
        s = {
            'creator': creator,
            'fee': '%s BEARS' % (delegation_fee_bear or required_fee_bear),
            ...
            'prefix': self.beard.chain_params["prefix"]
        }
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        """ Transfer BSD or BEARS to another account.

            :param str to: Recipient

            :param float amount: Amount to transfer

            :param str asset: Asset to transfer (``BSD`` or ``BEARS``)
        ...
        assert asset in ['@@000000021','@@000000013','BEARS', 'BSD', 'BEARS','BSD']

        if memo and memo[0] == "#":
            from bearbase import memo as Memo
            memo_wif = self.wallet.getMemoKeyForAccount(account)
            if not memo_wif:
                raise MissingKeyError("Memo key for %s missing!" % account)
            to_account = Account(to, beard_instance=self.beard)
    ...
    """
        ...
        op = operations.WithdrawVesting(
            **{
                ...
                "coining_shares":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=6, asset="COINS"),
            })
    ...
    def transfer_to_vesting(self, amount, to=None, account=None):
        ...
        :param float amount: number of BEARS to vest
    ...
    """
        ...
        op = operations.TransferToVesting(
            **{
                ...
                "amount":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=3, asset='BEARS')
            })

        return self.finalizeOp(op, account, "active")

    def convert(self, amount, account=None, request_id=None):
        """ Convert BearDollars to Bear (takes one week to settle)
        ...
        op = operations.Convert(
            **{
                ...
                "amount":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=3, asset='BSD')
            })

        return self.finalizeOp(op, account, "active")

    def transfer_to_savings(self, amount, asset, memo, to=None, account=None):
        """ Transfer BSD or BEARS into a 'savings' account.

            :param float amount: BEARS or BSD amount
            :param float asset: 'BEARS' or 'BSD'
    ...
    def transfer_from_savings(self,
                              ...
                              to=None,
                              account=None):
        """ Withdraw BSD or BEARS from 'savings' account.
    ...
    def claim_reward_balance(self,
        ...
        `reward_bear`, `reward_bsd` or `reward_vests`.

        Args:
            reward_bear (string): Amount of BEARS you would like to claim.
            reward_bsd (string): Amount of BSD you would like to claim.
        ...
        if none(
                float(first(x.split(' ')))
                for x in [reward_bsd, reward_bear, reward_vests]):
            a = Account(account)
            reward_bear = a['reward_bear_balance']
            reward_bsd = a['reward_bsd_balance']
            reward_vests = a['reward_vesting_balance']

        op = operations.ClaimRewardBalance(
            **{
                "account": account,
                "reward_bear": reward_bear,
                "reward_bsd": reward_bsd,
                "reward_vests": reward_vests,
            })
        return self.finalizeOp(op, account, "posting")

    def delegate_coining_shares(self, to_account, coining_shares,
                                account=None):
        ...
        op = operations.DelegateCoiningShares(
            **{
                "delegator": account,
                "delegatee": to_account,
                "coining_shares": str(Amount(coining_shares)),
            })
        return self.finalizeOp(op, account, "active")
    ...
    def witness_feed_publish(self,
        ...
        """ Publish a feed price as a witness.
            ...
            :param float bear_usd_price: Price of BEARS in USD (implied price)
    ...
    def interest(self, account):
        ...
        """
        account = Account(account, beard_instance=self.beard)
        last_payment = fmt_time_string(account["bsd_last_interest_payment"])
        next_payment = last_payment + timedelta(days=30)
        interest_rate = self.beard.get_dynamic_global_properties()[
                            "bsd_interest_rate"] / 100  # percent
        interest_amount = (interest_rate / 100) * int(
            int(account["bsd_seconds"]) / (60 * 60 * 24 * 356)) * 10 ** -3
    ...
    def set_withdraw_coining_route(self,
                                   to,
                                   percentage=100,
                                   account=None,
                                   auto_vest=False):
        """ Set up a vesting withdraw route. When vesting shares are
            ...
            :param bool auto_vest: Set to true if the from account
                ...
                receive them as BEARS. (defaults to ``False``)
        """
        if not account:
            account = configStorage.get("default_account")
        if not account:
        ...
        op = operations.SetWithdrawCoiningRoute(
            **{
                "from_account": account,
                "to_account": to,
                "percent": int(percentage * BEARS_1_PERCENT),
                "auto_vest": auto_vest
            })
    ...
    def allow(self,
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, beard_instance=self.beard)
    ...
    def disallow(self,
        ...
        try:
            pubkey = PublicKey(
                foreign, prefix=self.beard.chain_params["prefix"])
            affected_items = list(
    ...
    def rebear(self, identifier, account=None):
        """ Rebear a post
        ...
        op = operations.CommentOptions(
            **{
                ...
                "max_accepted_payout":
                    options.get("max_accepted_payout", default_max_payout),
                "percent_bear_dollars":
                    options.get("percent_bear_dollars", 100) * BEARSIT_1_PERCENT,
                "allow_votes":
                    options.get("allow_votes", True),
                "allow_curation_rewards":
                    options.get("allow_curation_rewards", True),
