...
from bearbase.exceptions import (InsufficientAuthorityError, MissingKeyError,
                                  InvalidKeyFormat)
from bearbase import operations
from bearbase.operations import Operation
from bearbase.transactions import SignedTransaction, fmt_time_from_now, \
    get_block_params
...
class TransactionBuilder(dict):
    ...
    def __init__(self,
        ...
        self.wallet = wallet_instance or Wallet(self.beard)
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(
                        authority[0], beard_instance=self.beard)
    ...
    def constructTx(self):
        ...
        ref_block_num, ref_block_prefix = get_block_params(self.beard)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.beard:
            operations.default_prefix = self.beard.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.beard.chain_params)
    ...
    def broadcast(self):
        ...
        try:
            if not self.beard.verify_authority(self.json()):
        ...
        try:
            self.beard.broadcast_transaction(self.json())
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        accountObj = Account(account, beard_instance=self.beard)
        ...
        for account_auth in authority["account_auths"]:
            account_auth_account = Account(
                account_auth[0], beard_instance=self.beard)
        ...
        self["blockchain"] = self.beard.chain_params
