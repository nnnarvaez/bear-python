...
from .instance import shared_beard_instance, stm
...
class Blockchain(object):
    ...
    Args:
        beard_instance (Beard): Beard() instance to use when accessing a RPC
        mode (str): `irreversible` or `head`. `irreversible` is default.
    """

    def __init__(self, beard_instance=None, mode="irreversible"):
        self.bear = beard_instance or shared_beard_instance()
    ...
    def info(self):
        ...
        """
        return self.bear.get_dynamic_global_properties()

    def config(self):
        return self.bear.get_config()
    ...
    def get_current_block(self):
        """ This call returns the current block
        """
        return self.bear.get_block(self.get_current_block_num())
    ...
    def stream_from(self,
        ...
        operations, return raw, unedited blocks as provided by beard. This
       ...
       """
        ...
        block_interval = self.config().get("BEARS_BLOCK_INTERVAL")
        ...
        while True:
            ...
            for block_num in range(start_block, head_block + 1):
                ...
                if full_blocks:
                    yield self.bear.get_block(block_num)
                elif batch_operations:
                    yield self.bear.get_ops_in_block(block_num, False)
                else:
                    for ops in self.bear.get_ops_in_block(block_num, False):
                        yield ops
    ...
    def reliable_stream(self,
        ...
        until getting a response from beard, allowing catching up after
        server downtime.

        Warnings: To ensure reliability, this method does some weird
        none-standard things with the beard client
        ...
        provided, will attempt to query beard for this value
        ...
        timeout (int): Time to wait on response from beard before assuming
        ...
        def get_reliable_client(_timeout):
            ...
            return stm.beard.Beard(
                nodes=self.bear.nodes,
                retries=1,
                timeout=_timeout,
                re_raise=True)
        ...
        def get_reliable_block_interval(_client):
            return reliable_query(_client, 'get_config',
                                  'database_api').get('BEARS_BLOCK_INTERVAL')
        ...
        for ops in self.stream_from(*args, **kwargs):
            ...
            for event in events:
                ...
                if not filter_by or op_type in filter_by:
                    # return unmodified beard output
    ...
    def get_all_usernames(self, *args, **kwargs):
        ...
        _ = args, kwargs
        warnings.warn(
            'Blockchain.get_all_usernames() is now Beard.get_all_usernames().'
        )
        return self.bear.get_all_usernames()
