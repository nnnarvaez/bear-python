...
from bearbase import bip38
from bearbase.account import PrivateKey
from bearbase.exceptions import (InvalidWifError, WalletExists)
...
class Wallet:
    """ The wallet is meant to maintain access to private keys for
        ...
        :param Bear rpc: RPC connection to a Bear node
        ...
        * **Wallet Database**: Here, bearlibs loads the keys from the
    ...
    def __init__(self, beard_instance=None, **kwargs):
        ...
        # Prefix
        if self.beard:
            self.prefix = self.beard.chain_params["prefix"]
        else:
        ...
        if "keys" in kwargs:
            ...
            """ If no keys are provided manually we load the SQLite
                keyStorage
            """
            from bearbase.storage import (keyStorage, KeyEncryptionKey)
            self.keyEncryptionKey = KeyEncryptionKey
            self.keyStorage = keyStorage

    def setKeys(self, loadkeys):
        """ This method is strictly only for in memory keys that are
            passed to Wallet/Bear with the ``keys`` argument
        """
        log.debug(
    ...
    def getOwnerKeyForAccount(self, name):
        ...
        if "owner" in Wallet.keyMap:
            ...
            account = self.beard.get_account(name)
            if not account:
                return
