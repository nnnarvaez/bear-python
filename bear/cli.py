...
from bearbase.account import PrivateKey
...
def legacyentry():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Command line tool to interact with the Bear network")
    """
    ...
    parser.add_argument(
        ...
        help='URL for public Bear API (default: "https://api.bearshares.com")'
    )
    ...
    parser.add_argument(
        ...
        version='%(prog)s {version}'.format(
            version=pkg_resources.require("bear")[0].version))
    ...
    parser_info = subparsers.add_parser(
        'info', help='Show basic BEARS blockchain info')
    ...
    """
        Command "transfer"
    """
    parser_transfer = subparsers.add_parser('transfer', help='Transfer BEARS')
    ...
    parser_transfer.add_argument(
        ...
        type=str,
        choices=["BEARS", "SBD"],
        help='Asset to transfer (i.e. BEARS or BSD)')
    parser_transfer.add_argument(
    ...
    parser_powerup = subparsers.add_parser(
        'powerup', help='Power up (vest BEARS as BEARS POWER)')
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing BEARS from bear POWER)')
    ...
    parser_powerdownroute.add_argument(
        ...
        type=str,
        default=configStorage["default_account"],
        help='The account receiving either VESTS/BearPower or BEARS.')
    parser_powerdownroute.add_argument(
        '--percentage',
        type=float,
        default=100,
    ...
    parser_powerdownroute.add_argument(
        ...
        help=('Set to true if the from account should receive the VESTS as'
              'VESTS, or false if it should receive them as BEARS.'))
    """
        Command "convert"
    """
    parser_convert = subparsers.add_parser(
        'convert',
        help='Convert BEARSDollars to Bear (takes a week to settle)')
    ...
    parser_buy = subparsers.add_parser(
        'buy', help='Buy BEARS or SBD from the internal market')
    ...
    parser_buy.add_argument(
        ...
        type=str,
        choices=["BEARS", "SBD"],
        help='Asset to buy (i.e. BEARS or BSD)')
    parser_buy.add_argument(
        'price', type=float, help='Limit buy price denoted in (SBD per BEARS)')
    parser_buy.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_sell = subparsers.add_parser(
        'sell', help='Sell BEARS or SBD from the internal market')
    ...
    parser_sell.add_argument(
        ...
        type=str,
        choices=["BEARS", "SBD"],
        help='Asset to sell (i.e. BEARS or BSD)')
    parser_sell.add_argument(
        'price',
        type=float,
        help='Limit sell price denoted in (SBD per BEARS)')
    parser_sell.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    """
        Command "rebear"
    """
    parser_rebear = subparsers.add_parser(
        'rebear', help='Rebear an existing post')
    parser_rebear.set_defaults(command="rebear")
    parser_rebear.add_argument(
        'identifier',
        type=str,
        help='author/permlink-identifier of the post to rebear')
    parser_rebear.add_argument(
        '--account',
        type=str,
        required=False,
        default=configStorage["default_account"],
        help='Rebear as this user (requires to have the ' +
    ...
    bear = stm.Bear(no_broadcast=args.no_broadcast, **options)
    ...
    if args.command == "set":
        ...
        if not args.objects:
            ...
            median_price = bear.get_current_median_history_price()
            bear_per_mvest = (
                    Amount(info["total_vesting_fund_bear"]).amount /
                    (Amount(info["total_vesting_shares"]).amount / 1e6))
            price = (Amount(median_price["base"]).amount / Amount(
                median_price["quote"]).amount)
            for key in info:
                t.add_row([key, info[key]])
            t.add_row(["bear per mvest", bear_per_mvest])
            ...
            # Account name
                ...
                try:
                    ...
                    for key in sorted(witness):
                        value = witness[key]
                        if key in ["props", "bsd_exchange_rate"]:
                            value = json.dumps(value, indent=4)
                        t.add_row([key, value])
                    print(t)
                    ...
                    pass
            # Public Key
            elif re.match("^SHR.{48,55}$", obj):
                account = bear.commit.wallet.getAccountFromPublicKey(obj)
                if account:
        ...
        if args.unsafe_import_key:
            ...
            while True:
                ...
                if not wifkey:
                    break
                try:
                    bear.commit.wallet.addPrivateKey(wifkey)
                except Exception as e:
                    print(str(e))
                    continue

                installed_keys = bear.commit.wallet.getPublicKeys()
                if len(installed_keys) == 1:
                    name = bear.commit.wallet.getAccountFromPublicKey(
                        installed_keys[0])
                    print("=" * 30)
                    print("Would you like to make %s a default user?" % name)
                    print()
                    print("You can set it with with:")
                    print("    bearpy set default_account <account>")
                    print("=" * 30)
        ...
        if confirm("Are you sure you want to delete keys from your wallet?\n"
            ...
            for pub in args.pub:
                bear.commit.wallet.removePrivateKeyFromPublicKey(pub)
        ...
        print(bear.commit.wallet.getPrivateKeyForPublicKey(args.pub))

    elif args.command == "listkeys":
        t = PrettyTable(["Available Key"])
        t.align = "l"
        for key in bear.commit.wallet.getPublicKeys():
            t.add_row([key])
        print(t)
        ...
        t.align = "l"
        for account in bear.commit.wallet.getAccounts():
            t.add_row([
        ...
        print_json(
            bear.commit.transfer_to_vesting(
                args.amount, account=args.account, to=args.to))

    elif args.command == "powerdown":
        print_json(
            bear.commit.withdraw_vesting(
                args.amount,
                account=args.account,
            ))

    elif args.command == "convert":
        print_json(bear.commit.convert(
            args.amount,
            account=args.account,
        ))
        ...
        print_json(
            bear.commit.set_withdraw_vesting_route(
                args.to,
        ...
        if args.account and isinstance(args.account, list):
            for account in args.account:
                ...
                print("\n%s" % a.name)
                t = PrettyTable(["Account", "BEARS", "SBD", "VESTS"])
                t.align = "r"
                t.add_row([
                    'Available',
                    a.balances['available']['BEARS'],
                    a.balances['available']['SBD'],
                    a.balances['available']['VESTS'],
                ])
                t.add_row([
                    'Rewards',
                    a.balances['rewards']['BEARS'],
                    a.balances['rewards']['SBD'],
                    a.balances['rewards']['VESTS'],
                ])
                t.add_row([
                    'Savings',
                    a.balances['savings']['BEARS'],
                    a.balances['savings']['SBD'],
                    'N/A',
                ])
                t.add_row([
                    'TOTAL',
                    a.balances['total']['BEARS'],
                    a.balances['total']['SBD'],
                    a.balances['total']['VESTS'],
                ])
                print(t)
        else:
            print("Please specify an account: bearpy balance <account>")
        ...
        if not args.foreign_account:
            from bearbase.account import PasswordKey
            pwd = get_terminal(
        ...
        if not args.key:
            ...
            args.key = format(memo_key.get_public_key(), "SHR")
            memo_privkey = memo_key.get_private_key()
            # Add the key to the wallet
            if not args.no_broadcast:
                bear.commit.wallet.addPrivateKey(memo_privkey)
        print_json(
            bear.commit.update_memo_key(args.key, account=args.account))
        ...
        print_json(
            bear.commit.create_account(
                args.accountname,
                creator=args.account,
                password=pw,
                delegation_fee_bear=args.fee,
            ))
        ...
        if args.file and args.file != "-":
            ...
            tx = sys.stdin.read()
        tx = eval(tx)
        print_json(bear.commit.sign(tx))
        ...
        print_json(
            bear.commit.approve_witness(args.witness, account=args.account))

    elif args.command == "disapprovewitness":
        print_json(
            bear.commit.disapprove_witness(
                args.witness, account=args.account))

    elif args.command == "rebear":
        print_json(bear.commit.rebear(args.identifier, account=args.account))
        ...
        print_json(
            bear.commit.update_account_profile(
        ...
        if args.account_creation_fee:
            props["account_creation_fee"] = str(
                Amount("%f BEARS" % args.account_creation_fee))
        if args.maximum_block_size:
            props["maximum_block_size"] = args.maximum_block_size
        if args.bsd_interest_rate:
            props["bsd_interest_rate"] = int(args.bsd_interest_rate * 100)

        print_json(
            bear.commit.witness_update(
        ...
        props = {
            "account_creation_fee":
                str(Amount("%f BEARS" % args.account_creation_fee)),
            "maximum_block_size":
                args.maximum_block_size,
            "bsd_interest_rate":
                int(args.bsd_interest_rate * 100)
        }
        print_json(
            bear.commit.witness_update(
    ...
    if op[0] == "vote":
        ...
        if memos:
            ...
            if len(memo) > 0 and memo[0] == "#":
                bear = shared_beard_instance()
                # memo = bear.decode_memo(memo, op[1]["from"])
                memo = bear.decode_memo(memo, op)
            str_ += " (%s)" % memo
        return str_
    elif op[0] == "interest":
...
def beartailentry():
    parser = argparse.ArgumentParser(
        description="UNIX tail(1)-like tool for the bear blockchain")
    parser.add_argument(
        '-f',
        '--follow',
